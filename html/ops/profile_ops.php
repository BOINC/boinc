...
if (array_key_exists('num', $_GET) && array_key_exists('set', $_GET)) {
    ...
    case 'approved':
        ...
        break;
    case 'unrated':
        $sql = "select * from profile, user WHERE profile.userid=user.id and (uotd_time IS NULL) AND (has_picture>0) and (verification = 0) and (user.total_credit>0) ORDER BY recommend DESC ";
        break;
    case 'rejected':
        $sql = $sql . "WHERE (uotd_time IS NULL) AND (verification = -1 ) ";
        break;
