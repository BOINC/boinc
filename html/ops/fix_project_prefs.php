...
// * rename allow_non_preferred_apps tag to allow_non_selected_apps
...
// replace allow_non_preferred_apps with allow_non_selected_apps
//
function handle_project_specific_block($node) {
    $anpa_node = $node->getElementsByTagName("allow_non_preferred_apps");
    if ($anpa_node->length == 1) {
        $new_node = $node->ownerDocument->createElement('allow_non_selected_apps', $anpa_node[0]->nodeValue);
        $node->replaceChild($new_node, $anpa_node[0]);
    }
...
function process_set($users) {
    ...
    foreach ($users as $user) {
        ...
        if (strstr($user->project_prefs, "app_id")
            ...
            || strstr($user->project_prefs, "allow_non_preferred_apps")) {
