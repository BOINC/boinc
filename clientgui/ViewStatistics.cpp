...
void CPaintStatistics::DrawAxis(wxDC &dc, const double max_val_y, const double min_val_y, const double max_val_x, const double min_val_x, 
	...
	dc.GetTextExtent(wxString::Format(wxT(" %s"), comma_print(max_val_y_all, 2)), &w_temp, &h_temp, &des_temp, &lead_temp);
	dc.GetTextExtent(wxString::Format(wxT(" %s"), comma_print(min_val_y_all, 2)), &w_temp2, &h_temp, &des_temp, &lead_temp);
...
//Draw val and lines
	...
	for (double ny = 0; ny <= double(d_oy_count); ++ny){
		dc.GetTextExtent(wxString::Format(wxT("%s"), comma_print(y_start_val + ny * d_oy_val, 2)), &w_temp, &h_temp, &des_temp, &lead_temp);
		...
		if ((y0 >= wxCoord(m_WorkSpace_Y_start)) && (y0 <= wxCoord(m_WorkSpace_Y_end))){
			...
			if (x0 < 0) x0 = 0;
			if (y0 < 0) y0 = 0;
			dc.DrawText(wxString::Format(wxT("%s"), comma_print(y_start_val + ny * d_oy_val, 2)), x0, y0);
		}
	}
...
void CPaintStatistics::DrawMarker(wxDC &dc) {
	if (m_GraphMarker1){
		...
		if ((x00 < wxCoord(m_Graph_X_start)) || (x00 > wxCoord(m_Graph_X_end)) ||
			...
			dc.SetTextForeground (m_pen_AxisYTextColour);
			dc.DrawText(wxString::Format(wxT("%s"), comma_print(m_GraphMarker_Y1, 2)) , x0, y00);
			dc.SetTextForeground (m_pen_AxisXTextColour);
