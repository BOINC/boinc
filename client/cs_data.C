...
extern void data_overflow_notify(PROJECT*);

// Polling function called in do_somthing()
//
...
// calculates the size of all the projects
...
int CLIENT_STATE::total_potential_self(PROJECT* p, double& tps) {
    ...
    for(i = 0; i < file_infos.size(); i++) {
        ...
        if(fip->ref_cnt == 0 && fip->project == p && fip->sticky) {
            if(!p->deletion_policy_expire) {
                tps += fip->nbytes;
            } else if(p->deletion_policy_expire && (fip->exp_date > time(0))) {
                tps += fip->nbytes;
            }
        }
    }
