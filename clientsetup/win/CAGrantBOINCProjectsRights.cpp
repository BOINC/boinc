...
public:
    virtual ~CAGrantBOINCProjectsRights() = default;
    explicit CAGrantBOINCProjectsRights(MSIHANDLE hMSIHandle) :
        BOINCCABase(hMSIHandle, _T("CAGrantBOINCProjectsRights"),
            _T("Validating the BOINC Projects group privilege levels")) {
    }

    UINT OnExecution() override final {
        PSID pSid;
        if (!GetAccountSid(nullptr, L"boinc_projects", &pSid)) {
...
UINT __stdcall GrantBOINCProjectsRights(MSIHANDLE hInstall) {
    return CAGrantBOINCProjectsRights(hInstall).Execute();
}
