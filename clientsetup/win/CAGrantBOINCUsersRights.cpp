...
public:
    virtual ~CAGrantBOINCUsersRights() = default;
    explicit CAGrantBOINCUsersRights(MSIHANDLE hMSIHandle) :
        BOINCCABase(hMSIHandle, _T("CAGrantBOINCUsersRights"),
            _T("Validating the BOINC Users group privilege levels")) {
    }

    UINT OnExecution() override final {
        PSID pSid;
        if (!GetAccountSid(nullptr, L"boinc_users", &pSid)) {
...
UINT __stdcall GrantBOINCUsersRights(MSIHANDLE hInstall) {
    return CAGrantBOINCUsersRights(hInstall).Execute();
}
