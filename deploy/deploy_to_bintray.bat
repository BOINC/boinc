...
if defined APPVEYOR_JOB_ID (
    echo Adding AppVeyor log to release notes...
    set BUILD_LOG=https://ci.appveyor.com/api/buildjobs/%APPVEYOR_JOB_ID%/log
    set data={"bintray": {"syntax": "markdown", "content": "%BUILD_LOG%"}}
    %CURL% -H Content-Type:application/json -X POST -d "%data%" "%API%/packages/%BINTRAY_REPO_OWNER%/%BINTRAY_REPO%/%pkg_name%/versions/%pkg_version%/release_notes"
)
