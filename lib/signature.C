...
int SIGNATURES::count() {
    return this->signatures.size();
}

int SIGNATURES::parse(XML_PARSER &xp) {
    SIGNATURE sig;
    int is_tag = false;
    int in_entry = false;
    int in_sig = false;
    ...
    while (!xp.get(tag, sizeof(tag), (bool&)is_tag)) {
        ...
        if (!is_tag) {
            printf("(SIGNATURES): unexpected text: %s\n", tag);
            continue;
        }
        if (in_entry) {
            if (!strcmp(tag, "/entry")) {
...
int SIGNATURES::parse_miofile_embed(MIOFILE &mf) {
    XML_PARSER xp(&mf);
    return this->parse(xp);    
}

int SIGNATURES::parse_file(const char* filename) {
    FILE* f;
    int retval;
...
int SIGNATURES::parse_buffer(char* buf) {
    MIOFILE mf;
    int retval;
...
int SIGNATURES::parse_buffer_embed(char* buf) {
    MIOFILE mf;
    char tag[4096];
    int is_tag;
    int s_found = false;
...
int SIGNATURES::write(MIOFILE &f, int max) {
    f.printf("<signatures>\n");
    for(int i=0;i < this->signatures.size(); i++) {
        f.append(max, "  <entry>\n");
        f.append(max, "    <thesignature>\n%s\n", this->signatures.at(i).signature);
        f.append(max, "    </thesignature>\n");
        f.append(max, "    <subject>%s</subject>\n", this->signatures.at(i).subject);    
        f.append(max, "    <type>%s</type>\n", (this->signatures.at(i).type == MD5_HASH) ? "md5" : "sha1");
        f.append(max, "    <hash>%s</hash>\n", this->signatures.at(i).hash);
        f.append(max, "  </entry>\n");
    }
    f.append(max, "</signatures>\n");
    return true;
}

// atisu: MIOFILE::printf was modified to increase buffer pointer
int SIGNATURES::write(MIOFILE &f) {
    if (this->signatures.size()==0) 
        return true;
    f.printf("<signatures>\n");
    for(int i=0;i < this->signatures.size(); i++) {
        f.printf("  <entry>\n");
        f.printf("    <thesignature>\n%s\n", this->signatures.at(i).signature);
        f.printf("    </thesignature>\n");
