...
if [ $result -eq 0 ]; then
    ...
    if [ "${builddocker_wrapper}" = "1" ]; then
        temp_style="Debug"
        if [ "${style}" = "Development" ]; then temp_style="Debug"; fi
        if [ "${style}" = "Deployment" ]; then temp_style="Release"; fi
        eval "xcodebuild -project ../samples/docker_wrapper/docker_wrapper.xcodeproj -target docker_wrapper -configuration ${temp_style} -sdk \"${SDKPATH}\" ${doclean} build  ${uselibcplusplus} ${theSettings}"
        result=$?
    fi
fi
