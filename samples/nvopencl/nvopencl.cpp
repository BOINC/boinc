...
// can be found at boinc\win_build\Build\Win32\Release\example_app_nvopencl.exe
...
char * convertToString(const char *fileName) {
	...
	FILE *infile=fopen(fileName,"r"); //look for nvopencl_kernels.cl in current directory which is in boinc\win_build
	if (!infile) { //not found 
		infile = fopen(KERNELS_FILEPATH,"r"); //look for nvopencl_kernels.cl in boinc\sample\nvopencl
		if (!infile) {
