...
if [ ! -d "$ANDROID_TC_ARMV6/arm-linux-androideabi" ]; then
    "$NDK_ARMV6_ROOT/build/tools/make-standalone-toolchain.sh" --verbose --toolchain=arm-linux-androideabi-clang --platform=android-16 --arch=arm --stl=gnustl --install-dir="$ANDROID_TC_ARMV6" "$@"
fi
