...
if [ "x$arch" = "x" ]; then
    ...
    echo '===== BOINC Client for ${arch} platform build start ====='
    android/build_component.sh --ci --component client --arch $arch --with-vcpkg
    echo '===== BOINC Client for ${arch} platform build done ====='
fi
