...
# check working directory because the script needs to be called like: ./android/ci_build_dependencies.sh
if [ ! -d "android" ]; then
...
if [ "x$arch" = "x" ]; then
    ...
    android/build_component.sh --ci --component dependencies
    echo '===== BOINC dependencies for all platforms build done ====='
else
    echo '===== BOINC dependencies for ${arch} platform build start ====='
    android/build_component.sh --ci --component dependencies --arch $arch
