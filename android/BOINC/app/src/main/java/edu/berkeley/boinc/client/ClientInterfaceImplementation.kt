 ...
 */
@Singleton
class ClientInterfaceImplementation @Inject constructor(private val clientStatus: ClientStatus) :
    RpcClient() {
    ...
    fun setGlobalPreferences(prefs: GlobalPreferences?): Boolean {
        ...
        if (workingPrefs != null) {
            clientStatus.prefs = workingPrefs
            return true
        }
        return false
    }
    ...
    fun readAuthToken(authFilePath: String): String {
        ...
        val authKeyLength = authKey.length
        logDebug(
            CLIENT,
            "Authentication key acquired. length: $authKeyLength"
        )
        return authKey
    }
    ...
    fun checkProjectAttached(url: String): Boolean {
        try {
            ...
            for (project: Project in attachedProjects) {
                logVerbose(CLIENT, project.masterURL + " vs " + url)
    ...
    fun lookupCredentials(credentials: AccountIn): AccountOut? {
        ...
        val success = lookupAccount(credentials) //asynch
        if (success) {
    ...
    fun transferOperation(transfers: List<Transfer>, operation: Int): Boolean {
        ...
        for (transfer: Transfer in transfers) {
            success = success && transferOp(operation, transfer.projectUrl, transfer.name)
            logDebug(CLIENT, "transfer: " + transfer.name + " " + success)
        }
        return success
    }
    ...
    fun getEventLogMessages(seqNo: Int, number: Int): List<Message> {
        ...
        var lowerBound: Int
        lowerBound = if (seqNo > 0) seqNo - number - 2 else messageCount - number - 1 // can result in >number results, if client writes message btwn. here and rpc.getMessages!

        // less than desired number of messsages available, adapt lower bound
        if (lowerBound < 0) lowerBound = 0
        val msgs = getMessages(lowerBound) // returns ever messages with seqNo > lowerBound
        if (seqNo > 0) {
    ...
    fun getAttachableProjects(
        ...
        val allProjectsList =
            allProjectsList // all_projects_list.xml
        ...
        if (allProjectsList == null) return emptyList()

        //filter projects that do not support Android
        for (candidate: ProjectInfo in allProjectsList) {
            // check whether already attached
            var alreadyAttached = false
            for (attachedProject: Project in attachedProjects) {
                if ((attachedProject.masterURL == candidate.url)) {
            ...
            for (supportedPlatform: String in candidate.platforms) {
    ...
    fun getProjectInfo(url: String): ProjectInfo? {
        val allProjectsList =
            allProjectsList // all_projects_list.xml
        for (tmp: ProjectInfo in allProjectsList) {
