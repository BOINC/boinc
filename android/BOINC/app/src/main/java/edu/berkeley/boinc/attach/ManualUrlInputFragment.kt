...
class ManualUrlInputFragment : DialogFragment() {
    private lateinit var urlInputET: EditText
    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val v = inflater.inflate(layout.attach_project_manual_url_input_dialog, container, false)
        urlInputET = v.findViewById(R.id.url_input)
        urlInputET.onFocusChangeListener = OnFocusChangeListener { view: View?, hasFocus: Boolean ->
    ...
    private fun checkDeviceOnline(): Boolean {
        ...
        if (!online) {
            val toast = Toast.makeText(
                getActivity(),
                string.attachproject_list_no_internet,
                Toast.LENGTH_SHORT
            )
            toast.show()
