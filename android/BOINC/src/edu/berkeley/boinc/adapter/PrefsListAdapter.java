	    		...
	    		if (item.status > 0) {
	    			...
	    			switch (item.unit) {
	    				...
	    				case SECONDS:
	    					...
	    					break;
	    				case CELSIUS:
	    					value = NumberFormat.getInstance().format(item.status) + this.activity.getString(R.string.prefs_unit_celcius);
	    					break;
	    				case MEGABYTES:
	    					value = Formatter.formatShortFileSize(this.activity, item.status.longValue() * 0x100000);
	    					break;
	    				case GIGABYTES:
	    					value = Formatter.formatShortFileSize(this.activity, item.status.longValue() * 0x40000000);
	    					break;
	    				default:
