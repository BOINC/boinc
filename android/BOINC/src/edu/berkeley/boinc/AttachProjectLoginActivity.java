...
public class AttachProjectLoginActivity extends Activity{
    ...
    public void onCreate(Bundle savedInstanceState) {  
        ...
        try {
        	...
        	if(Logging.DEBUG) Log.d(TAG,"projectInfo: " + projectInfo);
        	if(projectInfo != null) {
        		projectInfoPresent = true;
        		url = projectInfo.url; // set url field to information of projectInfo
        	}
        } catch (Exception e) {if(Logging.DEBUG) Log.d(TAG,"no project info...");}
	...
	private void populateLayout() {
		if(Logging.DEBUG) Log.d(TAG, "populateLayout");
		...
		if(projectConfig.userName) { // user vs. email?
			if(Logging.DEBUG) Log.d(TAG,"project is using user name instead of email for login");
	...
	public void register (View view) {
		if(Logging.DEBUG) Log.d(TAG, "register: " + view.getTag());
	...
	private final class GetProjectConfig extends AsyncTask<Void, Void, Integer> {
		...
		protected Integer doInBackground(Void... params) {
			try{
				if(!projectInfoPresent) { // only url string is available
					if(Logging.DEBUG) Log.d(TAG, "doInBackground() - GetProjectConfig for manual input url: " + url);
					...
					if(Logging.DEBUG) Log.d(TAG, "doInBackground() - GetProjectConfig for list selection url: " + projectInfo.url);
				...
				if (projectConfig != null && projectConfig.error_num != null && projectConfig.error_num == 0) {
					...
					if(Logging.DEBUG) Log.d(TAG,"getProjectConfig returned error num:" + projectConfig.error_num);
					return R.string.attachproject_login_error_toast;
				}
			} catch(Exception e) {
				if(Logging.WARNING) Log.w(TAG,"error in doInBackround",e);
				return R.string.attachproject_login_error_toast;
			}
		}
		...
		private void loadBitmap() {
			...
			try {
				...
				if(projectLogo!=null) if(Logging.DEBUG) Log.d(TAG, "logo download successful.");
			} catch (Exception e) {
				if(Logging.WARNING) Log.w(TAG,"loadBitmap failed.",e);
			}
		}
		
		private Boolean checkProjectAlreadyAttached(String url) {
			if(Logging.DEBUG) Log.d(TAG, "check whether project with url is already attached: " + url);
