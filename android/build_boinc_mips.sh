...
if [ -n "$CONFIGURE" ]; then
./_autosetup
./configure --host=mipsel-linux --with-boinc-platform="mipsel-android-linux-gnu" --with-ssl=$TCINCLUDES --with-curl=$TCINCLUDES --disable-server --disable-manager --disable-shared --enable-static
